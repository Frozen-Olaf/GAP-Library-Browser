...
public class CustomGoToDialog extends EscapableDialog {
     ...
     */
    public CustomGoToDialog(Dialog owner) {
        super(owner);
        init();
    }
     ...
     */
    public CustomGoToDialog(Frame owner) {
        super(owner);
        init();
    }
    ...
    private void init() {
        ...
        CornerRoundedTextPane crp = new CornerRoundedTextPane(new BorderLayout(), true);
        lineNumberField = crp.getTextField();
        lineNumberField.setColumns(16);
        ...
        crp.setTextFieldSize(lineNumberField.getPreferredSize().width, 26);
        
        JLabel label = UIUtil.newLabel(MSG, "LineNumber", lineNumberField);
        label.setPreferredSize(new Dimension(label.getPreferredSize().width, crp.getSize().height));
        label.setBorder(BorderFactory.createEmptyBorder(0, 0, 2, 0));
        enterLineNumberPane.add(label);
        enterLineNumberPane.add(Box.createHorizontalStrut(5));
        enterLineNumberPane.add(crp);
        ...
        setMinimumSize(new Dimension(179, 104));
    }
        ...
        private String fix(String str) {
            if (str != null) {
                ...
                if (origLength != str.length()) {
                    UIManager.getLookAndFeel().provideErrorFeedback(CustomGoToDialog.this);
                }
            }
            return str;
        }
